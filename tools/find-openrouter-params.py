...
# ///
"""find-openrouter-params — OpenRouter 모델별로 실제 max_tokens/max_completion_tokens
파라미터 조합을 탐색해서 engine/aimd/llm_params.json에 누적 저장한다.

사용법:
    export OPENROUTER_API_KEY=...     # 또는 LLM_API_KEY
    uv run tools/find-openrouter-params.py
    uv run tools/find-openrouter-params.py --models openrouter/free,anthropic/claude-sonnet-5

--models를 생략하면 DEFAULT_MODELS (openrouter/free) 고정 목록만 검사한다.
...
def main() -> int:
    ...
    if not api_key:
        print("OPENROUTER_API_KEY (or LLM_API_KEY) is not set", file=sys.stderr)
        return 1
